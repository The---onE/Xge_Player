...
BOOL WINAPI Mini_Proc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(uMsg)
	{
		...
		case WM_MOVE:
		{
			...
			x = (__int64)lParam % 65536;
			y = (__int64)lParam / 65536;
			//若纵坐标小于窗口吸附边缘最小距离
			if (y < NEAR_DISTANCE)
			{
				//设置鼠标离开窗口时应收缩
...
void Mini_OnHScroll(HWND hwnd, HWND hwndCtl, UINT code, int pos)
{
	switch(GetDlgCtrlID(hwndCtl))
	{
		...
		case IDC__SOUND_SLIDER:
		{
			_isSoundMove = TRUE;
			//得到音量滚动条位置
			int Voice= (int)SendMessage(GetDlgItem(hwnd,IDC__SOUND_SLIDER),TBM_GETPOS,0,0);//
			//将位置发送到主窗口
		...
		case IDC__MUSIC_SLIDER:
		{
			...
			{
				//得到播放进度滚动条位置
				int Time = (int)SendMessage(GetDlgItem(hwnd,IDC__MUSIC_SLIDER),TBM_GETPOS,0,0);//
				//将位置发送到主窗口
